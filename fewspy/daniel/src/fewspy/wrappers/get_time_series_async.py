...
def __result_async_to_time_series_set(async_result):
    ...
    if version is not None:
        time_zone = next((i for i in time_series_set_list if "timeZone" in i.keys()), None)
        if time_zone is not None:
            time_series = {"timeSeries": [i["timeSeries"][0] for i in time_series_set_list if "timeSeries" in i.keys()]}
